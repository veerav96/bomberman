...
#1 ==>ATTACK , 0 OTHERS
def attackStrategy(self,f_1,f_2,f_3,f_4,f_5,f_6,my_agent,isbomb,others,arena,bombs,explosion_map):
    if(f_1==4 and f_2==0 ):
        ...
        if(isbomb):
            ...
            crenemycurr=crenemyTOP=crenemyRight=crenemyDown=crenemyLeft=0
            crenemycurr = possibleCrenemy(self, f_3,f_4,f_5,f_6,arena,(x,y),others)
            if(f_3==0):
                crenemyTOP = possibleCrenemy(self, f_3,f_4,f_5,f_6,arena,(x,y-1),others)
            if(f_4==0):
                crenemyRight = possibleCrenemy(self, f_3,f_4,f_5,f_6,arena,(x+1,y),others)
            if(f_5==0):
                crenemyDown = possibleCrenemy(self, f_3,f_4,f_5,f_6,arena,(x,y+1),others)
            
            if(f_6==0):
                crenemyLeft = possibleCrenemy(self, f_3,f_4,f_5,f_6,arena,(x-1,y),others)

            maximum = max(crenemycurr,crenemyTOP, crenemyRight, crenemyDown, crenemyLeft)
            
            if(f_7!=5 and crenemycurr==maximum ): #escape possible, and max crenemies, throw bomb
                return 1

            if(crenemyTOP==maximum):
                return 2
            if(crenemyRight==maximum):
                return 3
            if(crenemyDown==maximum):
                return 4
            if(crenemyLeft==maximum):
                return 5
    return 0 #WAIT
    #crates/enemies
def possibleCrenemy(self, f3,f4,f5,f6,arena,pos,others):
    (x,y)=pos
    count=0
    for(i,j) in [(x+h,y) for h in range(0,4)] :
            if (0 <= i < arena.shape[0]) and (0 <= j < arena.shape[1]):
                if arena[i,j]!=-1 and arena[i,j]==1:
                    count+=1
                else:
                    break
    for(i,j) in [(x-h,y) for h in range(1,4)] :
            if (0 <= i < arena.shape[0]) and (0 <= j < arena.shape[1]):
                if arena[i,j]!=-1 and arena[i,j]==1:
                    count+=1
                else:
                    break
    for(i,j) in [(x,y+h) for h in range(1,4)] :
            if (0 <= i < arena.shape[0]) and (0 <= j < arena.shape[1]):
                if arena[i,j]!=-1 and arena[i,j]==1:
                    count+=1
                else:
...
def isunderDanger(self,my_agent,bombs,arena): 
    ...
    #use explosion map=1 not free and bombs with t=0 not free(not implemented), target nearest free tile
